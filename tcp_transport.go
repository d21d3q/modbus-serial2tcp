...
// based on https://github.dev/simonvetter/modbus
	...
	for {
		...
		if tt.lastTxnId != txnId {
			log.WithFields(log.Fields{
				"expected": tt.lastTxnId,
				"received": txnId,
			}).Debug("ignoring response with unexpected transaction id")
			continue
		}
...
func (tt *tcpTransport) readMBAPFrame() (p *pdu, txnId uint16, err error) {
	...
	if protocolId != 0x0000 {
		...
		log.WithFields(log.Fields{
			"expected": 0x0000,
			"received": protocolId,
		}).Debug("ignoring unexpected protocol id")
		return
	}
